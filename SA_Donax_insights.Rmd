...
The residual patterns across all scenarios demonstrate consistent behavior, with values oscillating around zero and remaining well-contained within the shaded confidence intervals throughout the temporal range. No systematic bias or temporal trends are evident in any of the four scenarios, indicating adequate model performance across the different minimum length thresholds ranging from 23 cm to 26 cm MLS, with the third scenario at 25 cm serving as the reference model. The estimated length distributions show remarkable consistency across all scenarios and throughout the study period, with modal patterns consistently concentrated around 2-3 mm. The distributional characteristics remain stable temporally within each scenario and show similar patterns between scenarios, suggesting that the different minimum length thresholds produce comparable outcomes in terms of the estimated length structure (Figure \@ref(fig:residuals)). 
...
This study addressed two key aspects to improve the assessment and management of the wedge clam (*Donax trunculus*) fishery in the Gulf of Cádiz, Spain. First, it represents the initial attempt to apply an integrated stock assessment model to this resource, combining multiple sources of information from the literature, official statistics, and a long-term monitoring program spanning more than a decade, which provided both fishery-dependent and fishery-independent data. Second, we used this model to evaluate alternative management strategies, with particular emphasis on potential changes to the minimum legal size. We assessed how such measures could influence the estimates of population variables derived from the stock assessment model, as well as the status and overall performance of the fishery. More broadly, stock assessment for this type of fishery involves significant conceptual and modeling challenges, which were explicitly considered in this study.


Generally, the stock assessment of this type of exploited species is carried out with direct methods or empiricals indicators [@huges1981; @Bouzaidi2020a; @Zhang2021; @Lamine2020; @Colakoglu2014; @Derbalietal2016]. Although these indicators have been widely used by local administration to promote proper management, the use of empirical indicators and their separate analysis can lead to erroneous interpretations, especially when conflicting signals arise [REFS]. However, this process has lacked appropriate metrics for assessing its dynamics, population variables, and clear reference points for management and decision-making. On the other hand, without clear, specific, and measurable reference points or management objectives over time are factors that can lead to the failure of fishery management and, consequently, the population [REFS @Punt2016]. In these sense, and taking advantage of the variety of data collected over the past ten years by official fishery statistical and monitoring programs, incorporating this information into an integrated stock assessment models is a next step in this fishery. Integrated stock assessment models are relatively recent for invertebrate and benthic species. 

Like most invertebrates, wedge clam is a species for which aging is complex [@Punt2013]. Therefore, a length-to-age model with age dynamics is used, which is transformed through a transition matrix based on the probabilities of age groups relative to the detailed structure. Stock assessment frameworks like length-to-age models have the capacity to reproduce the age dynamics of these populations and simultaneously transform them into population variables. There are exercises with indirect methods which have been applied in various groups and types of species where aging is difficult around the world, from gastropods [REF], bivalves[@Hernandez2011; @Canales2021], echinoderms[@Roa-Ureta2015; @Chen2003d; @Chen2003b] and cephalopods[REF]. 

To use a model-based approach for this species, it is necessary to establish a biological reference framework. In this context, most modern and internationally recognized fisheries management frameworks are based on the concept of Maximum Sustainable Yield (MSY) (Hilborn and Walters, 1992; Chen and Paloheimo, 1998; Walters, 1998). Within this background, modeling population dynamics through an integrated approach for the wedge clam allows reliable estimation of key parameters using appropriate statistical methods, providing estimates of state variables (e.g., biomass, abundance) and flow variables (e.g., fishing mortality, exploitation rates) for both the distribution area and the fishery, which can then be linked to MSY-based proxies. Proxies for BRPs have mostly been applied due to the difficulty of estimating MSY [@Hilborn2002; @Punt2013]. For example, initially to avoid growth overfishing, using F0.1 as the target BRP and Fmax as the limit BRP, based on yield per recruit [@Gabriel1975 and @Goodyear1993]. Then, to prevent recruitment overfishing, the use of a target BRP F40%SPR and a limit BRP F20%SPR is proposed, based on spawning biomass per recruit (Clark 1991 and 1993, Mace 1994, Mace and Sissenwine 1993). More recently, 40%SB0 (spawning biomass corresponding to 40% of virgin spawning biomass) has also been used as the target BRP and 20%SB0 as the limit (citation). Worldwide, reference frameworks for invertebrates fisheries have been proposed to give advice into management [@Baro2018; @Caddy1996d; @Caddy2004; @Canales2021; @FIPA2017; @Roa-Ureta2015; @Smith2004b; @Smith2004c]. For example, in Maine, it was estimated to use (F 0.1) as a reference [@Chen2003; @Chen2003b].  propose the use of BRPs based on spawning potential ratio (SPR) as a proxy for MSY according to  for California benthic fisheries. In this sense, the use of BRP that aim for MSY is proposed, according to the global fisheries scientific community.

In this study, we proposed a biological framework based on a depletion level of 20% of current spawning biomass (relative to virgin biomass or to the initial year of the historical series) as a biological reference point. This threshold is widely accepted internationally and is commonly used by Regional Fisheries Management Organizations (RFMOs) \[REF]. By adopting this level, we ensure alignment with the reference framework applied by ICES \[REF], which allows us to consistently identify and interpret the stock status of the wedge clam in the Gulf of Cádiz.

The estimated depletion levels under the four management scenarios tested were 10.2% for S1 (23 mm), 13.1% for S2 (24 mm), 14.6% for S3 (25 mm), and 16.1% for S4 (26 mm). All of these values remain below the proposed 20% threshold, indicating that the stock is currently above the limit reference point across scenarios. However, the increasing depletion levels observed when raising the minimum legal size suggest that higher size limits could enhance the long-term sustainability of the stock by maintaining a larger proportion of the spawning biomass. From a management perspective, these results highlight that adjusting the minimum legal size is an effective measure to influence the biological reference points and to ensure that the fishery operates safely above critical depletion thresholds.  

The current stock status reflects the cumulative impact of decades of intensive harvesting combined with the species' biological vulnerability. Wedge clams' short lifespan (4-5 years) and rapid growth make them particularly susceptible to recruitment overfishing, where fishing pressure prevents sufficient individuals from reaching reproductive maturity. The observed decline in spawning stock biomass since 2018 across all scenarios suggests that recent fishing mortality levels have exceeded the population productive capacity, despite existing management measures.
...
The temporal stability observed in both the residual diagnostics and length estimates across all four scenarios indicates robust model performance regardless of the minimum length scenario implemented.
...
While no statistically significant differences were detected between scenarios, the levels of key population variables for wedge clam differ across management alternatives. Furthermore, all four scenarios indicate an overfished status, whereby even minimal changes in management measures such as minimum legal size (MLS) have implications not only for stock status but also for fishery sustainability. Under overfished conditions, the population's reduced resilience means that subtle variations in management regulations can have disproportionate effects on recovery trajectories and long-term viability. This suggests that statistical non-significance should not be interpreted as management equivalence, particularly when the stock operates below sustainable thresholds where small management adjustments may determine whether the fishery can rebuild or faces further decline. The current overfished state emphasizes the critical need for precautionary management approaches, as the narrow margin between scenarios may represent the difference between stock recovery and continued depletion in this vulnerable population.
...
### Long-Term evaluation of curren management strategies


The long-term implications of current MLS regulations remain uncertain, mainly because the assessment period is still relatively short and the dynamics of biological, environmental, and socioeconomic factors are highly complex \[REF]. Although the current assessment provides useful insights into stock status and the management trade-offs involved, it is clear that ongoing monitoring and periodic reassessment will be essential to adaptively manage the fishery as productivity conditions change \[REF].

Looking ahead, the development of a formal management strategy evaluation (MSE) framework appears as a logical next step. Such an approach would provide a structured way to test alternative management strategies under uncertainty or under different productivity scenarios \[REF]. By doing so, it would allow managers and stakeholders to explore trade-offs between conservation and economic objectives over longer time horizons, ultimately supporting more robust decision-making \[REF]. This could well be the next step in advancing the management procedure of the wedge clam fishery.
...
knitr::include_graphics("Fig/summary_variables_7.jpeg")
```
...
# Figure 3.3: SSB time series
knitr::include_graphics("Fig/variables_summary_5.jpeg")
```

```{r deple-plot, fig.cap="Stock depletion trajectories relative to virgin biomass across four management scenarios from 2004-2025. The horizontal dashed line represents the 20% depletion threshold (0.2 Biomass Virgin)"}
# Figure 3.6: Kobe plot for stock status
knitr::include_graphics("Fig/depletion.jpeg")
```

```{r tabstatus}
ssb_status <- data.frame(
  Scenario = c("S1 (23mm)", "S2 (24mm)", "S3 (25mm)", "S4 (26mm)"),
  SSB_2024_tonnes = c("92.12", "124.10", "132.25", "140.57"),
  Depletion = c("10.2%", "13.1%", "14.6%", "16.1%"),
  Percent_Change_vs_25mm = c("-30.34%", "-6.16%", "Reference", "+6.29%"),
  P_SSB_decline_5yr = c("67.6%", "25.5%", "23.5%", "20.4%"),
  Stock_Status = c("Overfished", "Overfished", "Overfished", "Overfished")
)
...
raw_data <- read_excel("model_summary.xlsx")

comparison_table <- raw_data %>%
  pivot_longer(cols = -Model, names_to = "Metric", values_to = "Value") %>%
  pivot_wider(names_from = Model, values_from = Value) %>% 
  head(-2) %>%
  add_row(Metric = "EVI_norm_prom",
          base.model1 = 0.383,
          base.model2 = 0.425,
          base.model3 = 0.413,
          base.model4 = 0.391) %>%
  add_row(Metric = "risk_decline_prom",
          base.model1 = 236,
          base.model2 = 169,
          base.model3 = 163,
          base.model4 = 157) %>% 
  dplyr::filter(Metric != c("F_Fmsy",
                            "Depletion2024",
                            "SSB2024"))
...
knitr::include_graphics("Fig/riskplot.jpeg")
```
