...
The following equations provides a general mathematical expression of both biological and fishing processes that partly explain the population dynamics applicable to wedge clams. For more details, please refer to Supplement 1.
...
description <- c(
  "Alternative mode (MLS 2.3 fixed)",
...
comsum_violin_alt <- ggplot(err_long, aes(x = Model, y = Value, fill = Model)) +
  ...
  stat_summary(fun = median, geom = "point",
               size = 2, color = "black") +
  facet_wrap(~Variable, ncol = 4, scales = "free_y",
             labeller = labeller(Variable = c("Residuals" = "Residual Diagnostics",
                                             "Length" = "Length Estimates (mm)"))) +
  scale_fill_manual(name = "Scenario",
                    values = c("s1" = "#cccccc", 
                              "s2" = "#cccccc", 
                              "s3" = "#ca0020", 
...
dev_all <- bind_rows(dev1, dev2, dev3, dev4)

ggplot(dev_all, aes(x = Yr, y = dev, color = Scenario)) +
  geom_point(size = 1.5) +
  ...
  labs(x = "", y = "Residuals", title = "Recruitment residuals across scenarios") +
  ylim(-2, 2) +
  theme_few() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        legend.position = "bottom")
...
```{r}
s1 <- base.model1$sprseries$Deplete[1:20]
s2 <- base.model2$sprseries$Deplete[1:20]
s3 <- base.model3$sprseries$Deplete[1:20]
s4 <- base.model4$sprseries$Deplete[1:20]

years <- 2004:2023

deplete_df <- data.frame(
  Year = years,
  s1 = s1,
  s2 = s2,
  s3 = s3,
  ...
  pivot_longer(cols = starts_with("s"), names_to = "Scenario", values_to = "Depletion")


deple <- ggplot(deplete_df, aes(x = Year, y = Depletion, color = Scenario)) +
  geom_line(size = 1) +
...
```{r}
# Filtrar solo Fleet 1 y añadir etiqueta de escenario
lend1 <- base.model1$lendbase %>%
  filter(Fleet == 1) %>%
  mutate(escenario = "Scenario 1")
lend2 <- base.model2$lendbase %>%
  filter(Fleet == 1) %>%
  mutate(escenario = "Scenario 2")
lend3 <- base.model3$lendbase %>%
  filter(Fleet == 1) %>%
  mutate(escenario = "Scenario 3")
lend4 <- base.model4$lendbase %>%
  filter(Fleet == 1) %>%
  mutate(escenario = "Scenario 4")

# Combinar en un único data.frame
lend_all <- bind_rows(lend1, lend2, lend3, lend4)

# Calcular la talla media ponderada específica para cada año y escenario
...
lengthexp <- ggplot(lend_all,
       aes(x = Bin, y = factor(Yr), height = Exp, group = Yr)) +
  geom_ridgeline(fill = "lightgrey", color = "black", alpha = 0.6, scale = 10) +
...
```{r pearson, fig.height=6, fig.width=4, out.width="100%",  fig.cap="Pearson residual by scenario and fleet"}
...
df_all <- bind_rows(
  create_heatmap_df(base.model1$lendbase, "s1: 23 mm MLS"),
  create_heatmap_df(base.model2$lendbase, "s2: 24 mm MLS"),
  create_heatmap_df(base.model3$lendbase, "s3: 25 mm MLS (Ref Model)"),
  create_heatmap_df(base.model4$lendbase, "s3: 26 mm MLS")
)
...
```{r}
# ANOVA para Bio_all entre Model
anova_bio <- aov(Bio_all ~ Model, data = outpsall)
summary(anova_bio)

# ANOVA para Recruit_0 entre Model
anova_rec <- aov(Recruit_0 ~ Model, data = outpsall)
summary(anova_rec)
```

Biomass trend between scenarios.

```{r}
# Crear variable numérica
outpsall$Model_num <- as.numeric(factor(outpsall$Model, levels = c("s1","s2","s3","s4")))

fit_rec <- lm(Bio_all ~ Model_num, data = outpsall)
summary(fit_rec)

intercept <- coef(fit_rec)[1]
slope <- coef(fit_rec)[2]
pval <- summary(fit_rec)$coefficients[2,4]

ggplot(outpsall, aes(x = Model_num, y = Bio_all)) +
  geom_point(size = 2) +  
  geom_smooth(method = "lm", se = TRUE, color = "red") +  
  scale_x_continuous(breaks = 1:4, labels = c("s1","s2","s3","s4")) +
  labs(x = "", y = "Recruit", title = "") +
  annotate("text", x = 2.5, y = max(outpsall$Bio_all)*0.9,
           label = paste0("Intercept = ", round(intercept,1),
                          "\nSlope = ", round(slope,1),
...
#### Economics Vulnerability index

La lógica es:

1. **Ingresos** = precio × captura.
2. **Disponibilidad del recurso** = biomasa (proxy de sostenibilidad).
3. **Eficiencia/viabilidad** = ingresos ajustados por el estado del stock.

Un esquema sencillo podría ser:

$$
IVI_t = \frac{P_t \times C_t}{B_t}
$$
...
* $C_t$ = captura (kg),
* $B_t$ = biomasa (kg),
* $IVI_t$ se interpreta como **ingresos por unidad de biomasa disponible**, es decir, cuán “rentable” es el recurso considerando el stock.

También puedes normalizar el índice (ej: dividir por el valor máximo o por el promedio de los años) para que quede entre 0 y 1.



```{r}
# Función para calcular IVI para un modelo
calc_ivi <- function(model, nombre){
  datos <- data.frame(
    year    = 2004:2023,
    precio  = runif(20, 2, 5),   # €/kg (simulado, si tienes reales cámbialo)
    biomasa = model$timeseries$Bio_all[1:20],
    captura = model$timeseries$`obs_cat:_1`[1:20]
  )

  datos$ingresos <- datos$precio * datos$captura
  datos$IVI      <- datos$ingresos / datos$biomasa
  datos$IVI_norm <- datos$IVI / max(datos$IVI, na.rm = TRUE)
...
datos_all <- bind_rows(
  calc_ivi(base.model1, "s1"),
  calc_ivi(base.model2, "s2"),
  calc_ivi(base.model3, "s3"),
  calc_ivi(base.model4, "s4")
)
...
```{r}
# Calcular promedio de IVI normalizado por escenario
ivi_promedio <- datos_all %>%
  dplyr::group_by(Scenario) %>%
  dplyr::summarize(IVI_norm_prom = mean(IVI_norm, na.rm = TRUE))
...
1. Calcula la tasa de explotación:

   $$
   U_t = \frac{C_t}{B_t}
   $$

   (captura dividida por biomasa).

2. Define un **umbral crítico** (ejemplo: $U_{crit} = 0.3$).

3. El riesgo se estima como:

   $$
   Risk_t = \frac{U_t}{U_{crit}} \times 100
   $$

* Si $Risk_t < 100\%$ → bajo riesgo.
* Si $Risk_t = 100\%$ → justo en el umbral.
* Si $Risk_t > 100\%$ → alto riesgo de declive poblacional.

```{r}

# Función para calcular riesgo de declive
calc_risk <- function(model, nombre, Ucrit = 0.4){
  datos <- data.frame(
    year    = 2004:2023,
    biomasa = model$timeseries$Bio_all[1:20],
    captura = model$timeseries$`obs_cat:_1`[1:20]
  ...
  # Tasa de explotación
  datos$U <- datos$captura / datos$biomasa
  
  # Riesgo de declive (%)
  datos$risk_decline <- (datos$U / Ucrit) * 100
  datos$Scenario <- nombre
  return(datos)
}
...
risk_all <- bind_rows(
  calc_risk(base.model1, "s1"),
  calc_risk(base.model2, "s2"),
  calc_risk(base.model3, "s3"),
  calc_risk(base.model4, "s4")
)
...
```{r}
risk_promedio <- risk_all %>%
  dplyr::group_by(Scenario) %>%
  dplyr::summarize(risk_decline_prom = mean(risk_decline, 
                                            na.rm = TRUE))
...
tradeoff_data <- data.frame(
  ...
  SSB2024 = c(47.59, 64.36, 70.93, 78.36),
  EVI_norm_prom = c(0.38, 0.42, 0.41, 0.39),   
  Risk_decline = c(235.9, 168.7, 163.1, 156.5)             
)

library(ggrepel)

ggplot(tradeoff_data, aes(x = SSB2024, 
                          y = EVI_norm_prom, 
                          label = Scenario, 
                          size = Risk_decline, color = Scenario)) +
  ...
  theme_few() +
  theme(legend.position = "none") +
  scale_color_manual(values = c("s1 (23mm)" = "#cccccc", 
                                "s2 (24mm)" = "#999999",
                                "s3 (25mm)" = "#ca0020",
                                "s4 (26mm)" = "#666666"))
```
...
# Aplicar a los cuatro modelos
res <- rbind(
  extract_metrics(base.model1, "base.model1"),
  extract_metrics(base.model2, "base.model2"),
  extract_metrics(base.model3, "base.model3"),
  extract_metrics(base.model4, "base.model4")
)

# Exportar a Excel
write.xlsx(res, file = "model_summary.xlsx", rowNames = FALSE)
