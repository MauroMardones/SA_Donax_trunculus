...
densidad_path <- "~/IEO/FEMP_04/outputs/D15_17_26.csv"
...
dens1726 <- read.csv(densidad_path) %>%
  mutate(Date = as.Date(Date))
...
ggsave(
  final_index,
  filename = "Fig/Empirical_Index.png",
  width = 8,
  height = 4,
  dpi = 300
)
...
years_seq  <- seq(2013, 2025, by = 1)   # hasta 2025
year_lines <- as.Date(paste0(years_seq, "-01-01"))
...
recruitanom <- ggplot(
  anomalias_reclutamiento %>% filter(year <= 2025),
  aes(x = fecha, y = anomaly)
) +
  geom_col(aes(fill = signo)) +
  geom_vline(xintercept = year_lines,
  ...
  geom_smooth(method    = "loess",
              ...
              colour    = "gray",
              linewidth = 0.5) +
  scale_fill_manual(values = c("Positiva" = "red", "Negativa" = "black")) +
  scale_y_continuous(limits = c(-1, 1)) +
  theme_bw() +
  labs(x = "", y = "Juvenile proportion anomaly")
...
cpueanom <- ggplot(
  anomalias_cpue %>% filter(year <= 2025),
  aes(x = fecha, y = anomaly)
) +
  geom_col(aes(fill = signo)) +
  geom_vline(xintercept = year_lines,
...
chlanom <- ggplot(
  anomalias_chla %>% filter(year <= 2025),
  aes(x = fecha, y = anomaly)
) +
  geom_col(aes(fill = signo)) +
  geom_vline(xintercept = year_lines,
...
x_common <- scale_x_date(
  limits      = c(as.Date("2017-01-01"), as.Date("2025-12-31")),
  date_breaks = "1 month",
  date_labels = "%b-%Y",
  expand      = c(0, 0)
)
