...
ggsave(filename = file.path(fig_path,
                                        "Length_compositions_wedge_clam.jpg"),
       plot = lengyh1,
       width = 8,
       height = 5,
       units = "in",
...
# Calculate reference averages (assuming historical data available)
# Historical average: all years with available data
historical_avg <- ssb_data %>%
  filter(!is.na(Year) & !is.na(Value)) %>%
  summarise(avg_ssb = mean(Value, na.rm = TRUE)) %>%
  pull(avg_ssb)

# Average of the last 10 years: most recent years with data
...
p <- ggplot(ssb_data, aes(x = Year, y = Value, color = Scenario, fill = Scenario)) +
  ...
  # Horizontal reference lines with "aes(linetype)"
  geom_hline(aes(yintercept = historical_avg, linetype = "Historical average"), 
             color = "black", size = 1, alpha = 0.8) +
  geom_hline(aes(yintercept = recent_years, linetype = "Average last 10 years"), 
  ...
  labs(
    title = "",
    subtitle = paste0("Historical average: ", round(historical_avg, 1), 
                      " | Average last 10 years: ", round(recent_years, 1)),
    x = "",
    y = "Spawning Biomass (SSB)",
    color = "Scenario",
    fill = "Scenario",
    linetype = "Reference"
  ) +
...
calc_evi <- function(model, nombre){
  datos <- data.frame(
    year    = 2004:2024,
    precio  = runif(21, 2, 5),   # precios simulados
    biomasa = model$timeseries$Bio_all[1:21],
    captura = model$timeseries$`obs_cat:_1`[1:21]
  )
