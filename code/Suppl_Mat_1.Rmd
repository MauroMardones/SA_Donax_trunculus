...
dirb     <- here::here("EM","basecase")        # Initial model (2.2 MCRS fixed)
dir0    <- here::here("EM","s0")        # Scenario 2.2 (MCRS fixed)
dir1     <- here::here("EM","s1")        # Reference model (2.3 MCRS fixed)
# Alternative management scenarios
dir2 <- here::here("EM","s2")       # Scenario 2.4 (MCRS fixed)
dir3 <- here::here("EM","s3")       # Scenario 2.5 (MCRS fixed) (current MCRS)
dir4 <- here::here("EM","s4")       # Scenario 2.6 (MCRS fixed)
# # Simulated mean-based models
dir2.3 <- here::here("EM","s1_2.3")      # Simulated model: 2.3 mean MCRS
# dir2.4 <- here::here("s1_2.4")      # Simulated model: 2.4 mean MCRS
# dir2.5 <- here::here("s1_2.5")      # Simulated model: 2.5 mean MCRS (current MCRS)
# dir2.6 <- here::here("s1_2.6")      # Simulated model: 2.6 mean MCRS

# Output figure path
fig_path <- here::here("Fig")           # Directory for saving figures
exe <- here::here("executable", "ss3_opt_osx_arm64")
```
...
selfix <- ggplot() +
  ...
  geom_line(data = selectivity_data, 
            aes(x = length, y = selectivity, color = Scenario), size = 0.9) +
...
selfix
ggsave(filename = file.path(fig_path, "Scenarios.jpeg"),
       plot = selfix,
       width = 8,
       height = 3,
       units = "in",
...
description <- c(
   "22 mm. Strong reduction of current MCRS",
  "23 mm. Lower-than-current MCRS",
  "24 mm. Slightly below current MCRS",
  "25 mm. Current MCRS (baseline)",
  "26 mm. Above current MCRS"
)
...
directorios <- here::here("EM", c("s0", "s1", "s2", "s3", "s4"))
...
```{r data, warning=FALSE, message=FALSE, fig.cap="Data sources used in the stock assessment model for wedge clam in Gulf of Cádiz"}
...
```{r length, warning=FALSE, message=FALSE, fig.cap="Length compositions used in the stock assessment model for wedge clam in Gulf of Cádiz"}
...
lengyh1 <- ggplot(dat_long, aes(x = Talla, 
                     y = factor(year),
                     height = Frecuencia, 
                     group = year,
                     fill = factor(year))) +
...
ggsave(filename = "../Fig/Length_compositions_wedge_clam.jpg",
       plot = lengyh1,
       width = 8,
       height = 5,
       units = "in",
...
# 1) Crear datos relativos a la media, manteniendo tus colores/escenarios
# Relativo a la media para todas excepto Depletion
# Escalar 0-1 para todas excepto Depletion (que queda absoluta)
all_df_rel <- all_df %>%
  ...
  dplyr::mutate(
    ...
    Value = dplyr::case_when(
      Variable == "Depletion" ~ Value,
      range_ref > 0 ~ (Value - min_ref) / range_ref,
      TRUE ~ 0
    ),
    sd = dplyr::case_when(
      Variable == "Depletion" ~ sd,
      range_ref > 0 ~ sd / range_ref,
      TRUE ~ 0
    )
  ) %>%
  dplyr::ungroup() %>%
  dplyr::select(-min_ref, -max_ref, -range_ref)

plot_var <- function(df, var_name) {
  y_lab <- ifelse(var_name == "Depletion",
                  "Depletion",
                  paste0(var_name, " (Relative)"))

  p <- ggplot(df %>% filter(Variable == var_name),
              aes(x = Year, y = Value, color = Scenario, fill = Scenario)) +
    geom_line(linewidth = 1) +
    geom_ribbon(aes(ymin = Value - sd, ymax = Value + sd), alpha = 0.2, color = NA) +
    labs(y = y_lab, x = "") +
...
# Graficar
plot_recr <- plot_var(all_df_rel, "Recruitment")
plot_ssb  <- plot_var(all_df_rel, "SSB")
plot_spr  <- plot_var(all_df_rel, "SPRratio")
plot_depl <- plot_var(all_df_rel, "Depletion")
...
ggarrange(plot_recr,
          ...
          plot_depl,
          ncol = 3, nrow = 1,
          common.legend = TRUE, legend = "bottom") # save
ggsave(filename = "../Fig/summary_variables_10.jpeg",
       width = 10,
       height = 3,
       units = "in",
       dpi = 300)
...
variables_to_plot <- c("Recruitment",
                       "SSB",
                       "Depletion")
...
This Figure \@ref(fig:pearson) provides a diagnostic assessment of the statistical performance of alternative model scenarios in fitting length composition data across years, fleets, and model assumptions. Across the four model scenarios, representing a baseline (2.5 cm as reference) and three values of Minimum Conservation Reference Size (MCRS: 2.3 cm, 2.4 cm, and 2.6 cm), the reference model generally shows balanced and relatively small residuals across both commercial and populational fleets, suggesting a good fit to the observed data. However, as MCRS increases, particularly in the 2.4 cm and 2.6 cm scenarios, a pattern of larger and more systematic residuals emerges, especially within the populational fleet. These patterns are most evident from 2016 onwards and are concentrated in mid-size length bins, indicating a potential mismatch between the model's structural assumptions and the actual population dynamics under stricter MCRS regimes. Specifically, the models may be underestimating the proportion of intermediate-size individuals, suggesting limitations in capturing shifts in selectivity, growth, or recruitment processes. The commercial fleet, in contrast, shows more consistent residual behavior across all scenarios, which may imply a better alignment between model assumptions and observed exploitation dynamics. Temporally, early years (2013–2015) show relatively small and scattered residuals, but residual magnitude and clustering increase in later years, reinforcing the need to consider temporal variability in key processes. Overall, this residual analysis indicates a decline in model fit quality with increasing MCRS, highlighting the need for more flexible model structures—such as time-varying selectivity or refined growth and recruitment models—to improve biological realism and ensure robust predictions in management strategy evaluations.
...
Model performance differed across scenarios (Figure \@ref(fig:rmse3)). RMSE values (left panel) were lowest under s0 and increased progressively through s1–s4, with higher medians and greater variability at larger MCRS values. Scenario s3 (25 mm MCRS, reference case) showed intermediate RMSE values relative to the other scenarios. Residual distributions (right panel) were centered around zero in all cases, with similar overall patterns across scenarios.
...
Hindcast validation for `s1` (Figure \@ref(fig:hcval1))

```{r hcval1, fig.cap="Hindcast validation for s1 by fleet"}
par(mfrow=c(1,2), mar=c(5,4,2,1))
...
Hindcast validation for `s2` (Figure \@ref(fig:hcval2))

```{r hcval2, fig.cap="Hindcast validation for s2 by fleet"}
par(mfrow=c(1,2), mar=c(5,4,2,1))
...
Hindcast validation for `s3` (Figure \@ref(fig:hcval3))

```{r hcval3, fig.cap="Hindcast validation for s3 by fleet"}
par(mfrow=c(1,2), mar=c(5,4,2,1))
...
Hindcast validation for `s4` (Figure \@ref(fig:hcval4))

```{r hcval4, fig.cap="Hindcast validation for s4 by fleet"}
par(mfrow=c(1,2), mar=c(5,4,2,1))
...
# save in ouputs folder
write.csv(diag_tidy, "../outputs/Model_Diagnosis_Results.csv", row.names = FALSE)
```
...
Statistical differences between scenarios in terms of biomass and recruitment were evaluated using analysis of variance (ANOVA). The ANOVA tests whether the means of these variables differ significantly across the different model scenarios. The results are summarized in Table \@ref(tab:anovaresults). Used `tabla_resumen`object in the previous section that contains the mean values of key population indicators (SSB, Recruitment, Depletion, SPRratio) for each scenario.

```{r anovaresults, message=FALSE, warning=FALSE, fig.cap ="ANOVA results for key population indicators"}
...
ggarrange(plotlist = plots_noleg,
          ncol = 3, nrow = 1)
ggsave(filename = "../Fig/anova_scene.jpeg", width = 8, height = 3, dpi = 300)
```
...
extract_metrics <- function(model, name, ref_model = NULL){
  
  # Valores SSB y Depletion en 2024 (último año de la serie)
  ...
  # Reclutamiento: CV como medida de variabilidad
  reclut       <- model$timeseries$Recruit_0
  reclut_valid <- reclut[reclut > 0 & !is.na(reclut)]
  cv_recruit   <- sd(reclut_valid) / mean(reclut_valid) * 100
  
  # Años bajo percentil 25 del modelo de referencia (s3)
  # Si se pasa ref_model se usa su distribución como baseline
  if (!is.null(ref_model)) {
    reclut_ref  <- ref_model$timeseries$Recruit_0
    reclut_ref  <- reclut_ref[reclut_ref > 0 & !is.na(reclut_ref)]
    threshold   <- quantile(reclut_ref, 0.25)
  } else {
    threshold   <- quantile(reclut_valid, 0.25)
  }
  low_recruit_pct <- mean(reclut_valid < threshold) * 100
  ...
  data.frame(
    ...
    CV_Recruitment     = cv_recruit,
    LowRecruitment_pct = low_recruit_pct,
    Bio2024            = bio2024
  )
}
...
# Modelo de referencia (s3 = MCRS 25 mm)
ref_model <- models_list$s3

# Extraer métricas usando s3 como baseline para low recruitment
metrics_list <- lapply(names(models_list), function(x){
  extract_metrics(models_list[[x]], x, ref_model = ref_model)
})

res_final <- bind_rows(metrics_list)

# Etiquetas legibles para las métricas
metric_labels <- c(
  SSB2024            = "SSB 2024 (t)",
  Depletion2024      = "Depletion 2024 (B/B0)",
  B_Bmsy             = "B/Bmsy",
  Decline5yr_pct     = "SSB decline last 5 yr (%)",
  CV_Recruitment     = "CV Recruitment (%)",
  LowRecruitment_pct = "Years below ref. P25 (%)",
  Bio2024            = "Total biomass 2024 (t)"
)
...
# Tabla
kbl(res_wide,
    caption = "Summary of key population metrics across MCRS scenarios",
    digits  = 2,
  ...
  # Resaltar fila de depletion (fila 2) como métrica clave
...
write.xlsx(res_wide, file = "../outputs/model_summary_wide.xlsx", rowNames = FALSE)
```
