...
```{r echo=FALSE}
dir01<-here("s01")# Modelo test
dir1<-here("s1")# Reference model
dir2<-here("s2") # landing 2
dir3<-here("s3") # landing 3
dir4<-here("s4") # landing 4
dir2.3<- here("s1_2.3")
dir2.4<- here("s1_2.4")
dir2.5<- here("s1_2.5")
dir2.6<- here("s1_2.6")
...
ggplot(selectivity_data, aes(x = length, y = selectivity, color = L50)) +
  geom_line(size = 1.2) +
  labs(
    title = "",
    x = "Shell length (mm)",
...
descripcion<-c("Solo Desembarque e Índice",
               "Flota comercial y poblacional, Desembarque, Indice, Composiciones de Tallas",
...
SSplotTimeseries(base.model2.5,
                 subplot = 7)
```
